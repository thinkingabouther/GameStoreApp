    ...
    public class DBController
    {
        ...
        private static GameParams ExtractGameParams(Game game)
        {
            ...
            using (var db = new GameStoreDBContext())
            {
                ...
                var currentDuration = (from duration in db.Duration
                                       where game.Duration_ID == duration.ID
                                       select duration).First();
                currentGame.minDuration = currentDuration.Lowest_duration;
                currentGame.maxDuration = currentDuration.Highest_duration;
                var currentPlayers = (from players_number in db.Players_number
                                      where game.Player_number_ID == players_number.ID
                                      select players_number).First();
                currentGame.minPlayers = currentPlayers.Lowest_players_number;
                currentGame.maxPlayers = currentPlayers.Highest_players_number;
                                                                ...
                                                                string genre, string type)
        {
            ...
            using (var db = new GameStoreDBContext())
            {
                var selectedGames = from game in db.Game
                                    ...
                                    maxPrice >= game.Price &&
                                    minPrice <= game.Price &&
                                    minDuration <= (from duration in db.Duration 
                                                    where duration.ID == game.Duration_ID 
                                                    select duration.Lowest_duration).FirstOrDefault() &&
                                    maxDuration >= (from duration in db.Duration 
                                                    where duration.ID == game.Duration_ID 
                                                    select duration.Highest_duration).FirstOrDefault() &&
                                    minDifficulty <= game.Difficulty &&
                                    maxDifficulty >= game.Difficulty &&
                                    minPlayers <= (from players in db.Players_number 
                                                   where players.ID == game.Player_number_ID 
                                                   select players.Lowest_players_number).FirstOrDefault() &&
                                    maxPlayers >= (from players in db.Players_number 
                                                   where players.ID == game.Player_number_ID 
                                                   select players.Highest_players_number).FirstOrDefault()
                                    select game;
                if (genre != "Не указано")
                {
            ...
            using (var db = new GameStoreDBContext())
            {
                ...
                switch (propertyName)
                {
                    ...
                    case "minDuration":
                        ...
                        {
                            Lowest_duration = int.Parse(propertyValue),
                            Highest_duration = ExtractGameParams(currentGame).maxDuration
                        };
                        db.Duration.Add(newDuration);
                        currentGame.Duration_ID = newDuration.ID;
                        break;
                    case "maxDuration":
                        var newMaxDuration = new Duration
                        {
                            Lowest_duration = ExtractGameParams(currentGame).minDuration,
                            Highest_duration = int.Parse(propertyValue)
                        };
                        db.Duration.Add(newMaxDuration);
                        currentGame.Duration_ID = newMaxDuration.ID;
                        break;
                    case "Genre":
                        break;
                    case "minPlayers":
                        var newMinPlayers = new Players_number
                        {
                            Lowest_players_number = int.Parse(propertyValue),
                            Highest_players_number = ExtractGameParams(currentGame).maxPlayers
                        };
                        db.Players_number.Add(newMinPlayers);
                        currentGame.Player_number_ID = newMinPlayers.ID;
                        break;
                    case "maxPlayers":
                        var newMaxPlayers = new Players_number
                        {
                            Lowest_players_number = ExtractGameParams(currentGame).minPlayers,
                            Highest_players_number = int.Parse(propertyValue)
                        };
                        db.Players_number.Add(newMaxPlayers);
                        currentGame.Player_number_ID = newMaxPlayers.ID;
                        break;
                    case "Type":
                        break;
        ...
        public static string GetPropertyValue(int gameID, string propertyName)
        {
            using (var db = new GameStoreDBContext())
            {
                ...
                switch (propertyName)
                {
                    ...
                    case "minDuration":
                        return (from duration in db.Duration
                                where duration.ID == currentGame.Duration_ID
                                select duration).First().Lowest_duration.ToString();
                    case "maxDuration":
                        return (from duration in db.Duration
                                where duration.ID == currentGame.Duration_ID
                                select duration).First().Highest_duration.ToString();
                    case "Difficulty":
                        return currentGame.Difficulty.ToString();
                    case "minPlayers":
                        return (from players in db.Players_number
                                where players.ID == currentGame.Player_number_ID
                                select players).First().Lowest_players_number.ToString();
                    case "maxPlayers":
                        return (from players in db.Players_number
                                where players.ID == currentGame.Player_number_ID
                                select players).First().Lowest_players_number.ToString();
        ...
        public static bool AddGameToDB(GameParams game)
        {
            using (var db = new GameStoreDBContext())
            {
                ...
                var existingDuration = (from duration in db.Duration
                                        where duration.Highest_duration == game.maxDuration &&
                                              duration.Lowest_duration == game.minDuration
                                        select duration);
                Duration currentDuration;
                if (existingDuration.Count() > 0)
                {
                    currentDuration = existingDuration.First();
                }
                else
                {
                    currentDuration = new Duration
                    {
                        Lowest_duration = game.minDuration,
                        Highest_duration = game.maxDuration
                    };
                    db.Duration.Add(currentDuration);
                }
                var currentDurationID = currentDuration.ID;
                var existingPlayers = (from players in db.Players_number
                                       where players.Highest_players_number == game.maxPlayers &&
                                             players.Lowest_players_number == game.minPlayers
                                       select players);
                Players_number currentPlayers;
                if (existingPlayers.Count() > 0)
                {
                    currentPlayers = existingPlayers.First();
                }
                else
                {
                    currentPlayers = new Players_number()
                    {
                        Lowest_players_number = game.minPlayers,
                        Highest_players_number = game.maxPlayers
                    };
                    db.Players_number.Add(currentPlayers);
                }
                var currentPlayersID = currentPlayers.ID;
                var currentGame = new Game
                {
                    Name = game.Name,
                    Duration_ID = currentDurationID,
                    Player_number_ID = currentPlayersID,
